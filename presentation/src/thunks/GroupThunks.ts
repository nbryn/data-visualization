...
export const updateGroupViewData = (): ThunkAction<void, RootState, null, Action<string>> => async (dispatch) => {
   ...
   result.lastYearLineChartData = DataMappingService.mapLastYearData(groupsLastYear, true);
   result.lastYearBarChartData = DataMappingService.mapLastYearData(groupsLastYear, false);
   result.lastMonthBarChartData = DataMappingService.mapLastMonthData(groupsLastMonth);
...
export const updateGroupSearchData = (group: string): ThunkAction<void, RootState, null, Action<string>> => async (
   dispatch
) => {
...
export const updateNGOGroupsData = (ngo: string): ThunkAction<void, RootState, null, Action<string>> => async (
   dispatch
) => {
